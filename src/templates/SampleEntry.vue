  ...
  <Layout>
    <section>
      <div class="grid md:grid-cols-2 gap-4 h-auto">
        ...
        <div class="grid">
          <BuildConfig
            :config="$page.sample.template"
            v-if="$page.sample.template"
          />
        </div>
        ...
        <div class="place-self-end justify-self-start flex">
          <span class="self-center mr-2"
            >You find the source of {{ $page.sample.title }} here:</span
          >
          <span>
            <a :href="$page.sample.git_url" rel="noopener" target="_blank">
              <svg
